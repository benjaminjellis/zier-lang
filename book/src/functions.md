...
- defines a function `square`, that takes one arg `x`
- arguments to functions live inside the curly brackets `{}`
- The body of the function is then defined in the second set of round brackets. This is how you invoke functions in `Opal`, using Polish Notation. The function comes first then the argument
...
Note that no type signature is required (Opal doesn't actually support them apart from when calling directly into Erlang). `Opal` uses a Hindley-Milner type system that infers types at compile time.
