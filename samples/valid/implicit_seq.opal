...
(let bind {m f} (match m (Ok x) ~> (f x) (Error e) ~> (Error e)))
...
(let log_and_compute {x y z}
  ...
  (safe_div y z)
  (let? [a (safe_div x y) b (safe_div a z)] (Ok b)))

;; implicit sequencing in let body
(let compute {x y z} (let [a (safe_div x y)] (safe_div x z) a))

(let main {} (log_and_compute 100 5 4))
